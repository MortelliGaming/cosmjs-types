...
export const Params = {
  ...
  fromJSON(object: any): Params {
    ...
    if (Array.isArray(object?.authorizedTickSpacing))
      obj.authorizedTickSpacing = object.authorizedTickSpacing.map((e: any) => BigInt(e.toString()));
    if (Array.isArray(object?.authorizedSpreadFactors))
      obj.authorizedSpreadFactors = object.authorizedSpreadFactors.map((e: any) => String(e));
    if (isSet(object.balancerSharesRewardDiscount))
    ...
    if (Array.isArray(object?.authorizedUptimes))
      obj.authorizedUptimes = object.authorizedUptimes.map((e: any) => Duration.fromJSON(e));
    if (isSet(object.isPermissionlessPoolCreationEnabled))
    ...
    if (Array.isArray(object?.unrestrictedPoolCreatorWhitelist))
      obj.unrestrictedPoolCreatorWhitelist = object.unrestrictedPoolCreatorWhitelist.map((e: any) =>
        String(e),
      );
    if (isSet(object.hookGasLimit)) obj.hookGasLimit = BigInt(object.hookGasLimit.toString());
    return obj;
  },
